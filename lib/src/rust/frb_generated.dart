...
class RustLib extends BaseEntrypoint<RustLibApi, RustLibApiImpl, RustLibWire> {
  ...
  @override
  int get rustContentHash => 437516840;
...
abstract class RustLibApi extends BaseApi {
  ...
  Future<List<DwcBundleColumn>> crateApiDwcDwcBundleColumns();
...
class RustLibApiImpl extends RustLibApiImplPlatform implements RustLibApi {
  ...
  Future<List<DwcBundleColumn>> crateApiDwcDwcBundleColumns() {
    return handler.executeNormal(
      NormalTask(
        ...
        codec: SseCodec(
          decodeSuccessData: sse_decode_list_dwc_bundle_column,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiDwcDwcBundleColumnsConstMeta,
        argValues: [],
        apiImpl: this,
      ),
    );
  ...
  TaskConstMeta get kCrateApiDwcDwcBundleColumnsConstMeta =>
      const TaskConstMeta(debugName: "dwc_bundle_columns", argNames: []);
  ...
  DwcBundleColumn dco_decode_dwc_bundle_column(dynamic raw) {
    ...
    if (arr.length != 4)
      throw Exception('unexpected arr length: expect 4 but see ${arr.length}');
    return DwcBundleColumn(
      table: dco_decode_String(arr[0]),
      header: dco_decode_String(arr[1]),
      termUri: dco_decode_String(arr[2]),
      profile: dco_decode_String(arr[3]),
    );
  }
  ...
  List<DwcBundleColumn> dco_decode_list_dwc_bundle_column(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_dwc_bundle_column).toList();
  }
  ...
  DwcBundleColumn sse_decode_dwc_bundle_column(SseDeserializer deserializer) {
    ...
    var var_termUri = sse_decode_String(deserializer);
  ...
  List<DwcBundleColumn> sse_decode_list_dwc_bundle_column(
    ...
    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <DwcBundleColumn>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_dwc_bundle_column(deserializer));
    }
    return ans_;
  }
  ...
  void sse_encode_dwc_bundle_column(
    ...
    sse_encode_String(self.termUri, serializer);
  ...
  void sse_encode_list_dwc_bundle_column(
    ...
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_dwc_bundle_column(item, serializer);
    }
  }
